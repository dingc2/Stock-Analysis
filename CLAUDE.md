...
```
data/           DataProvider ABC (yfinance impl), cache wrappers, Quote dataclass
...
ml/             ModelProvider ABC: xgboost_direction.py, lstm_direction.py
...
**XGBoost:** max_depth=2, n_estimators=150, lr=0.03, subsample=0.6, colsample=0.5, early stopping (10 rounds).
...
- Model persistence/caching (avoid retraining every page load)
